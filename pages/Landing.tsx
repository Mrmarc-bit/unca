  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        ...
        // 2. Cek Database (Akses tabel events)
        // Menggunakan count header-only untuk efisiensi
        const { error: dbError, count } = await supabase
            .from('events')
            .select('*', { count: 'exact', head: true });

        if (dbError) {
            // Kode error 42P01 berarti tabel tidak ditemukan (koneksi sukses, tapi belum setup SQL)
            if (dbError.code === '42P01') {
                setDbStatus({ 
                    status: 'warning', 
                    message: 'Terhubung, tapi Tabel belum dibuat. Silakan jalankan script SQL.' 
                });
            } else {
                throw dbError;
            }
            ...
            setDbStatus({ 
                status: 'connected', 
                message: `Terhubung ke Supabase. (${count} events found)` 
            });
        }
      } catch (err: any) {
        ...
        if (msg === 'Failed to fetch') msg = 'Koneksi Internet/Network Error';
        if (msg.includes('JWT')) msg = 'API Key Salah / Invalid';
        
        setDbStatus({ status: 'error', message: msg });
      }
    };
  ...
  return (
    <div className="overflow-hidden relative">
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2">
          {showDebug && (
             <div className="bg-slate-900 text-slate-200 p-4 rounded-xl text-xs font-mono shadow-2xl max-w-xs mb-2 animate-in slide-in-from-bottom-5">
                <p className="font-bold text-white mb-2 border-b border-slate-700 pb-1">System Status</p>
                ...
                <p className="mt-1 text-slate-400 leading-tight">{dbStatus.message}</p>
             </div>
          )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="grid md:grid-cols-3 gap-8">
              ...
              <Card className="p-8 hover:-translate-y-2 transition-transform duration-300">
                 ...
                 <h3 className="text-xl font-bold mb-3 text-slate-900">Registrasi Simpel</h3>
